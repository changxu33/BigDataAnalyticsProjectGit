...
for(i in 76:97){
  ...
  rf=randomForest(f1 , data = train , subset = 1:nrow(train),ntree = 35, mtry = 16)
  plot(rf)
  #Get Training error
  print(names(train)[i])
...
for(i in c(78:78)){
  ...
  indexd <- c(indexa, indexb, indexc[1:(10*(length(indexa)+length(indexb)))])
  #print(length(indexd))
  errVec <- c(0,0,0,0,0,0)
  print(names(train)[i])
  #Create the formula
...
for(i in c(76:97)){
  ...
  indexa <- which(train[,i-22] == 0 & train[,i] == 1)
  indexb <- which(train[,i-22] == 1 & train[,i] == 0)
  print(length(indexa) + length(indexb))
}
