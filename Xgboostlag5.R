...
for(i in 142:163){
  ...
  xgb =xgboost(data = train[,1:141], label = train[,i], max_depth = 2, eta = 1, nrounds = 2, min_child_weight = 1,objective = "binary:logistic", verbose = 0)
  #Get Training error
  print(names(dt)[i])
...
for(i in c(142:163)){
  ...
  indexd <- c(indexa, indexb, indexc[1:as.integer(10*(length(indexa)+length(indexb)))])
  #print(length(indexd))
  errVec <- c(0,0,0,0,0,0)
  print(names(dt[i]))
  
  xgb =xgboost(data = data.matrix(trainDT[indexd,1:141]), label = data.matrix(trainDT[indexd,i]), max_depth = 5, eta = .7, nrounds = 4, min_child_weight = 1,objective = "binary:logistic", verbose = 0)
  prediction <- predict(xgb, train)
  prediction <- prediction >.5
  #Training Error
...
importance_matrix <- xgb.importance(names(dt), model = xgb)
xgb.plot.importance(importance_matrix, 25)
