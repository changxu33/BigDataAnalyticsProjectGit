...
for(i in c(76:97)){
  ...
  indexd <- c(indexa, indexb, indexc[1:as.integer(9*(length(indexa)+length(indexb)))])
  #print(length(indexd))
  errVec <- c(0,0,0,0,0,0)
  print(names(dt[i]))
  
  xgb =xgboost(data = data.matrix(trainDT[indexd,1:75]), label = data.matrix(trainDT[indexd,i]), max_depth = 3, eta = .5, nrounds = 16, min_child_weight = 1.5,objective = "binary:logitraw", verbose = 0)
  prediction <- predict(xgb, train)
  prediction <- prediction >.5
  #Training Error
